...
export const askAi = async (messages) => {
  try {
    ...
    const model = isOpenAi ? "gpt-4o-mini" : "openrouter/free";

    console.log(
      `AI Request: Using ${isOpenAi ? "OpenAI" : "OpenRouter"} (${model})`,
    );
    ...
    const isQuotaError =
      errorData.code === "insufficient_quota" || statusCode === 429;
    const isAuthError = statusCode === 401 || statusCode === 403;

    if (isQuotaError || isAuthError) {
      console.warn(
        "Falling back to DYNAMIC MOCK response due to Auth/Quota issue.",
      );
